 ...
 * @date    2025-12-14
 *
 * Tests the optional helper functions in safetimer_helpers.h
 */
...
void test_create_started_invalid_parameters(void) {
    ...
    h = safetimer_create_started(0x80000000UL, TIMER_MODE_REPEAT, test_callback, NULL);
    TEST_ASSERT_EQUAL(SAFETIMER_INVALID_HANDLE, h);
...
void test_create_started_pool_exhaustion(void) {
    ...
    for (i = 0; i < MAX_TIMERS; i++) {
        handles[i] = safetimer_create_started(1000, TIMER_MODE_REPEAT, test_callback, NULL);
        TEST_ASSERT_NOT_EQUAL(SAFETIMER_INVALID_HANDLE, handles[i]);
    }

    /* Next creation should fail */
    handles[MAX_TIMERS] = safetimer_create_started(1000, TIMER_MODE_REPEAT, test_callback, NULL);
...
void test_create_started_no_resource_leak(void) {
    ...
    safetimer_handle_t h = safetimer_create_started(1000, TIMER_MODE_REPEAT, test_callback, NULL);
    TEST_ASSERT_NOT_EQUAL(SAFETIMER_INVALID_HANDLE, h);
...
void test_create_started_multiple_timers(void) {
    ...
    h1 = safetimer_create_started(100, TIMER_MODE_REPEAT, test_callback, &counts[0]);
    h2 = safetimer_create_started(200, TIMER_MODE_REPEAT, test_callback, &counts[1]);
    h3 = safetimer_create_started(300, TIMER_MODE_REPEAT, test_callback, &counts[2]);
...
void test_create_started_batch_partial_failure(void) {
    ...
    int created = safetimer_create_started_batch(MAX_TIMERS + 2, 500, TIMER_MODE_REPEAT,
                                                  callbacks, data, handles);
...
void test_create_started_batch_null_checks(void) {
    ...
    created = safetimer_create_started_batch(3, 500, TIMER_MODE_REPEAT,
                                              NULL, NULL, handles);
    TEST_ASSERT_EQUAL_INT(0, created);
}
