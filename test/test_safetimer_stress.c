...
void test_stress_all_timers_active_simultaneously(void) {
  ...
  for (i = 0; i < MAX_TIMERS; i++) {
    ...
    handles[i] = safetimer_create(period, TIMER_MODE_REPEAT, counting_callback,
                                  &callbacks_fired[i]);
...
void test_stress_long_running_timer_10_days(void) {
  safetimer_handle_t h;
  const uint32_t TEN_DAYS_MS =
      10UL * 24UL * 60UL * 60UL * 1000UL; /* 864,000,000 ms */
  ...
  g_callback_count = 0;
  h = safetimer_create(TIMER_PERIOD_MS, TIMER_MODE_REPEAT, stress_callback,
                       NULL);
  TEST_ASSERT_NOT_EQUAL(SAFETIMER_INVALID_HANDLE, h);
  TEST_ASSERT_EQUAL_INT(TIMER_OK, safetimer_start(h));
...
void test_stress_timer_32bit_wraparound_boundary(void) {
  ...
  mock_bsp_advance_time(
      20); /* This will cause wraparound: 0xFFFFFFF0 + 20 = 0x00000004 */
...
#else
  TEST_PASS_MESSAGE("Skipping 32-bit overflow test in 16-bit mode");
#endif
}
...
void test_stress_multiple_timers_different_periods_long_run(void) {
  ...
  h_fast =
      safetimer_create(100, TIMER_MODE_REPEAT, counting_callback, &count_fast);
  h_medium = safetimer_create(500, TIMER_MODE_REPEAT, counting_callback,
                              &count_medium);
  h_slow =
      safetimer_create(2000, TIMER_MODE_REPEAT, counting_callback, &count_slow);
