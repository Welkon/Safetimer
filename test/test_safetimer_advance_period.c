...
void test_advance_period_basic_phase_locked(void) {
  ...
  safetimer_handle_t h =
      safetimer_create(100, TIMER_MODE_REPEAT, test_advance_callback, NULL);
...
void test_advance_vs_set_period_behavior(void) {
  ...
  safetimer_handle_t h_advance =
      safetimer_create(100, TIMER_MODE_REPEAT, NULL, NULL);
  TEST_ASSERT_NOT_EQUAL(SAFETIMER_INVALID_HANDLE, h_advance);

  /* Timer B: uses set_period (resets from current) */
  safetimer_handle_t h_set =
      safetimer_create(100, TIMER_MODE_REPEAT, NULL, NULL);
...
void test_advance_period_invalid_params(void) {
  ...
  TEST_ASSERT_EQUAL(TIMER_ERR_INVALID,
                    safetimer_advance_period(h, 0x80000000UL));
