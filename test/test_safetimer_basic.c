 ...
 *                     -I../test/unity ../src/safetimer.c
 * ../test/mocks/mock_bsp.c \
 *                     ../test/unity/unity.c test_safetimer_basic.c -o
 * test_runner
 */
...
void test_create_valid_timer_one_shot(void) {
  ...
  /* Handle is opaque, cannot verify range against index */
}
...
void test_delete_timer_allows_slot_reuse(void) {
  ...
  /* Handles will NOT be equal due to generation counter increment (ADR-005) */
...
void test_timer_handles_32bit_wraparound(void) {
  ...
  safetimer_start(
      handle); /* Expire time = 0xFFFFFFF0 + 100 = 0x00000054 (wraps) */
