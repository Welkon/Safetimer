...
/* Disable harmless compiler warnings for production code */
...
static int32_t safetimer_tick_diff(bsp_tick_t lhs, bsp_tick_t rhs) {
  ...
  signed short diff_s16 = (signed short)diff_u16;  /* Use signed short instead of int16_t (C89 compatible) */
...
static void update_expire_time(int slot_index, bsp_tick_t current_tick) {
  ...
  index = (uint8_t)(slot_index & 0xFFU); /* Explicit range limit */
...
void safetimer_process(void) {
  ...
  for (i = 0; i < (uint8_t)MAX_TIMERS; i++) {
    ...
    if (safetimer_tick_diff(current_tick, timer->expire_time) >= 0) {
      bsp_enter_critical();
...
safetimer_err_t safetimer_get_remaining(safetimer_handle_t handle,
  ...
  current_tick = bsp_get_ticks();
  diff = safetimer_tick_diff(timer->expire_time, current_tick);
