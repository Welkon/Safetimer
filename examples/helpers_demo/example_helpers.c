 ...
 * @date    2025-12-14
 *
 * This example demonstrates the convenience helpers in safetimer_helpers.h
 * and compares them with the core API.
 */
...
void led1_blink(void *user_data) {
    (void)user_data;
    printf("[%5lu ms] LED1 blink (500ms period)\n", g_system_ticks);
}

void led2_blink(void *user_data) {
    (void)user_data;
    printf("[%5lu ms] LED2 blink (1000ms period)\n", g_system_ticks);
}

void led3_blink(void *user_data) {
    (void)user_data;
    printf("[%5lu ms] LED3 blink (2000ms period)\n", g_system_ticks);
}

void delayed_task(void *user_data) {
    (void)user_data;
    printf("[%5lu ms] Delayed task executed!\n", g_system_ticks);
}
...
void example_core_api(void) {
    ...
    safetimer_handle_t stage1 = safetimer_create(1000, TIMER_MODE_ONE_SHOT, delayed_task, NULL);
    safetimer_handle_t stage2 = safetimer_create(2000, TIMER_MODE_ONE_SHOT, delayed_task, NULL);

    if (stage1 == SAFETIMER_INVALID_HANDLE || stage2 == SAFETIMER_INVALID_HANDLE) {
...
void example_helper_api(void) {
    ...
    safetimer_handle_t led1 = safetimer_create_started(500, TIMER_MODE_REPEAT, led1_blink, NULL);
    safetimer_handle_t led2 = safetimer_create_started(1000, TIMER_MODE_REPEAT, led2_blink, NULL);
    safetimer_handle_t led3 = safetimer_create_started(2000, TIMER_MODE_REPEAT, led3_blink, NULL);
...
void example_macro_helper(void) {
    ...
    SAFETIMER_CREATE_STARTED_OR(heartbeat, 1000, TIMER_MODE_REPEAT, led1_blink, NULL, {
...
void example_comparison(void) {
    ...
    printf("  safetimer_handle_t h = safetimer_create(500, TIMER_MODE_REPEAT, cb, NULL);\n");
    printf("  if (h != SAFETIMER_INVALID_HANDLE) {\n");
    printf("      safetimer_start(h);\n");
    printf("  }\n\n");

    printf("Helper API (convenience):\n");
    printf("  safetimer_handle_t h = safetimer_create_started(500, TIMER_MODE_REPEAT, cb, NULL);\n\n");
