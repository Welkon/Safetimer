 ...
 * @license MIT
 *
 * @note This header provides optional convenience functions for common usage patterns.
 *       Include this ONLY if you want simplified APIs. For explicit control, use
 *       the core safetimer.h API directly.
 *
 * @par Usage Example:
 * @code
 * // Option 1: Use helpers for immediate-start scenarios (most common)
 * #include "safetimer_helpers.h"
 * led_timer = safetimer_create_started(500, TIMER_MODE_REPEAT, blink_led, NULL);
 *
 ...
 *
 * This helper combines safetimer_create() and safetimer_start() into a single
 * call for the common case where you want a timer to begin immediately.
 *
 * @par Typical Use Cases:
 * - Periodic tasks (LED blinking, sensor polling, heartbeats)
 * - Simple one-shot delays
 * - Timers that always start at system initialization
 *
 * @par NOT Suitable For:
 * - Cascaded/chained timers (use explicit create + conditional start)
 * - Timers started based on runtime conditions
 * - Timers that need to be created but remain dormant
 *
 * @par Error Handling:
 * If creation succeeds but start fails, the timer is automatically deleted
 * to prevent resource leaks. This ensures the returned handle is either
 * fully functional or INVALID.
 *
 * @par Bare-Metal Safety:
 * Unlike RTOS timer APIs (e.g., FreeRTOS xTimerStart), SafeTimer's start
 * operation is immediate and local (no scheduler queue). Callbacks only
 * fire when safetimer_process() is called, making immediate start predictable
 * and safe even during initialization.
 *
 ...
 *
 * @note This is a zero-overhead inline function (no Flash/RAM cost)
 * @note Equivalent to: create() → check → start() → check → cleanup on failure
 *
 * @code
 * // Example: Three periodic LED blinkers
 * safetimer_handle_t led1 = safetimer_create_started(500, TIMER_MODE_REPEAT, blink1, NULL);
 * safetimer_handle_t led2 = safetimer_create_started(1000, TIMER_MODE_REPEAT, blink2, NULL);
 * safetimer_handle_t led3 = safetimer_create_started(2000, TIMER_MODE_REPEAT, blink3, NULL);
 *
 * if (led1 == SAFETIMER_INVALID_HANDLE) {
 *     // Handle error: both create and start failure cases covered
 * }
 * @endcode
 *
 * @see safetimer_create()
 * @see safetimer_start()
 * @see safetimer_delete()
 */
static inline safetimer_handle_t safetimer_create_started(
    uint32_t            period_ms,
    timer_mode_t        mode,
...
)
{
    ...
    safetimer_handle_t handle = safetimer_create(period_ms, mode, callback, user_data);
...
/**
 * @brief Create and start multiple timers with identical parameters (batch helper)
 *
 * This helper simplifies creating several timers with the same configuration
 * but different callbacks/user_data.
 *
 * @param count      Number of timers to create (must be ≤ remaining pool slots)
 ...
 *
 * @return Number of timers successfully created and started
 *
 * @note Partial success is possible (returns < count). Check individual handles.
 * @note If any creation/start fails, that handle will be SAFETIMER_INVALID_HANDLE
 *
 * @code
 ...
 * @endcode
 */
static inline int safetimer_create_started_batch(
    int                 count,
    uint32_t            period_ms,
    timer_mode_t        mode,
 ...
 * SAFETIMER_CREATE_STARTED_OR(led_timer, 500, TIMER_MODE_REPEAT, blink_led, NULL, {
 ...
 * @endcode
 */
#define SAFETIMER_CREATE_STARTED_OR(handle, period_ms, mode, callback, user_data, error_handler) \
    do { \
        (handle) = safetimer_create_started((period_ms), (mode), (callback), (user_data)); \
        if ((handle) == SAFETIMER_INVALID_HANDLE) { \
            error_handler \
        } \
    } while (0)
